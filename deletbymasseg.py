...
async def main():
    # 1. تحديد اسم ملف جلسة قصير لـ Pyrogram ليقوم بإنشائه (مثلاً "my_bot_session")
    # 2. تمرير سلسلة الجلسة الموجودة لديك كـ "session_string"
    async with Client(
        "my_bot_session",  # هذا هو اسم ملف الجلسة الذي سيتم إنشاؤه
        api_id=API_ID,
        api_hash=API_HASH,
        session_string=SESSION # هنا تمرر سلسلة الجلسة المخزنة مسبقاً
    ) as client:
